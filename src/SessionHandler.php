 ...
 * Thise release includes a fixes for the following:
...
final class SessionHandler implements \SessionHandlerInterface
{
    ...
    public function __construct(PDO $pdo, string $tableName, int $maxAge = 86400)
    {
        ...
        $createSessionsTableQuery = "
            CREATE TABLE IF NOT EXISTS {$this->dbTable} (
                ...
                `data` mediumtext,
                PRIMARY KEY (`id`),
                KEY `modified_timestamp` (`modified_timestamp`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
    ...
    {
        $sql = "
            REPLACE INTO {$this->dbTable} (id, data) 
            VALUES(:id, :data)";
        $stmt = $this->dbConnection->prepare($sql);
